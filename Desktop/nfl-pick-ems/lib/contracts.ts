...
export class RealNFLPickEmsContract implements NFLPickEmsContract {
  ...
  async submitPicks(week: number, picks: string[]): Promise<{ success: boolean; hash: string }> {
    try {
      ...
      const entryFee = BigInt(2000000) // $2 USDC (6 decimals) - corrected from earlier mistake
  ...
  private convertPicksToBitmask(picks: string[]): bigint {
    ...
    // Convert picks array to bitmask format
    // Each pick in the array corresponds to a game index
    // If pick is 'home', set bit at that position to 1
    // If pick is 'away', leave bit at that position as 0
    picks.forEach((pick, gameIndex) => {
      if (pick === 'home') {
        bitmask |= BigInt(1) << BigInt(gameIndex)
      }
      // 'away' picks are represented by 0 bits, so no action needed
    })
    
    console.log('Converted picks to bitmask:', {
      picks,
      bitmask: bitmask.toString(),
      bitmaskHex: '0x' + bitmask.toString(16),
      bitCount: this.countBits(bitmask)
    })
  ...
  async getEntryFee(): Promise<bigint> {
    // Entry fee is hardcoded in the contract as 2_000_000 (2 USDC with 6 decimals)
