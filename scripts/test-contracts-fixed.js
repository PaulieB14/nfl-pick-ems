...
async function main() {
  ...
  // Test 6: Create a test week with proper lock time (minimum 1 hour buffer)
  const weekId = 1;
  const gameCount = 16; // 16 games in a typical NFL week
  const currentTime = Math.floor(Date.now() / 1000);
  const lockTime = currentTime + 7200; // 2 hours from now (more than 1 hour buffer)
  ...
  const gameIndex = 0;
  const gameStartTime = currentTime + 3600; // 1 hour from now
